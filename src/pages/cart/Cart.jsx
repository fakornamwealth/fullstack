...
const Cart = () => {
  ...
  const getCart = async () => {
    try {
      ...
      const total = data.reduce(
        (total, currentItem) =>
          (total = Number(total) + Number(currentItem.amount)),
        0
      );
      setCart(data);
      setTotal(total);
  ...
  // ca;l the getCart function when the page loads/renders
  useEffect(() => {
    getCart();
  }, []);
  ...
  const updateCart = async (cart) => {
    ...
    try {
      ...
      const { data } = await userRequest.put(`/cart/${cart._id}`, {
        ...
        amount: cart.amount * quantity, //amount = cart amount multiply by quantity
      });
      getCart();
    } catch (error) {
      console.log(error);
